...
Main() {
	case $RELEASE in
		...
		bionic)
			...
			;;
		focal)
			mkdir /tmp/gpib-build
			cd /tmp/gpib-build
			cp -r /tmp/overlay/linux-gpib-kernel-4.3.0 ./
			cd /tmp/gpib-build/linux-gpib-kernel-4.3.0
			ls -la /lib/modules/
			LINUX_SRCDIR=/lib/modules/5.7.19-sunxi/build make && LINUX_SRCDIR=/lib/modules/5.7.19-sunxi/build make install
			cd /tmp
			rm -rf /tmp/gpib-build
			cp /tmp/overlay/blacklist-xradio_wlan.conf /etc/modprobe.d/
			depmod -a 5.7.19-sunxi
			cp -r /tmp/overlay/linux-gpib-user-4.3.0 /tmp/linux-gpib-user-4.3.0
			cd /tmp/linux-gpib-user-4.3.0 && ./configure && make && make install
			cd /tmp
			rm -rf /tmp/linux-gpib-user-4.3.0
			touch /root/.no_rootfs_resize
			echo "opi-gpib" > /etc/hostname
			cp /tmp/overlay/gpib.conf /etc/gpib.conf
			ldconfig
			echo "raspi_gpib" >> /etc/modules-load.d/modules.conf
			cp /tmp/overlay/gpib-tcp-server.py /usr/local/bin/
			chmod +x /usr/local/bin/gpib-tcp-server.py
			cp /tmp/overlay/gpib-tcp-server.service /etc/systemd/system/gpib-tcp-server.service
			cp /tmp/overlay/gpib-tcp-server-configure.service /etc/systemd/system/gpib-tcp-server-configure.service
			ln -s /etc/systemd/system/gpib-tcp-server-configure.service /etc/systemd/system/multi-user.target.wants/gpib-tcp-server-configure.service
			ln -s /etc/systemd/system/gpib-tcp-server@.service /etc/systemd/system/multi-user.target.wants/gpib-tcp-server.service
			echo "127.0.2.1 opi-gpib" >> /etc/hosts
			cp /tmp/overlay/avahi-daemon.conf /etc/avahi/avahi-daemon.conf
			;;
	esac
} # Main
